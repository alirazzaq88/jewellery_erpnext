...
class OperationMetalLoss(Document):
	def before_save(self):
		self.update_gross_weights()
		
	def update_gross_weights(self):
		filters = [
			["Job Card","operation","=",self.operation],
			["Job Card","modified","Between",[self.date_from,self.date_to]], 
			["Job Card","docstatus","=",1]
		]

		data = frappe.db.get_list('Job Card', filters=filters, fields=["loss_gold_weight", "loss_finding_weight", "metal_purity"])
		...
		self.gold_loss = self.gross_pure_metal_loss - (self.loss_recovered or 0)	
